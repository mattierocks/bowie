...
var labels2 = ['Extra', 'Voice', 'Lead', 'Cameo', 'Actor'];
...
svgSingles
  ...
  .attr('x', function(d, i) {
    return i * (width / singles.length) + (width / singles.length - padding) / 2;
  })
  .attr('y', function(d) {
    return height;
  })
...
svgActing
  ...
  .attr('text-anchor', 'middle')
  .attr('x', function(d, i) {
    return i * (width / acting.length) + (width / acting.length - padding) / 2;
  })
  .attr('y', function(d) {
    return height2;
  })
